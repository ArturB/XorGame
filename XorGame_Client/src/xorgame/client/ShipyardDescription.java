...
/**
 * Panel with descritpion of particular ship or defense system. Contains its cost and building time. 
 * @author Artur
 */
public class ShipyardDescription extends javax.swing.JPanel {
    Main main;
    ShipyardItem item;
    /**
     * Creates new form ShipyardDescription
     */
    public ShipyardDescription() {
        initComponents();
    }
    ...
    /**
     * Sets item name and icon. 
     * @param item_ Item to show. 
     * @param iconPath Path to icon of the item. 
     */
    ...
    /**
     * Updates description of the item - its cost and building time. Downloads data from the game server. 
     */
    public void updateDescription() {
        try {
            ...
            MCostLabel.setText(NumberFormat.getNumberInstance(Locale.US).format(main.getMainControler().shipCost(item, Material.M)));
            KCostLabel.setText(NumberFormat.getNumberInstance(Locale.US).format(main.getMainControler().shipCost(item, Material.K)));
            DCostLabel.setText(NumberFormat.getNumberInstance(Locale.US).format(main.getMainControler().shipCost(item, Material.D)));
            ACostLabel.setText(NumberFormat.getNumberInstance(Locale.US).format(main.getMainControler().shipCost(item, Material.A)));
            timeLabel.setText(main.getMainControler().shipTime(main.getSelectedPlanet().getPlanetPK(), Location.P, item, 1));
    ...
    /**
     * Starts building of new item. Checks item technical requirements and input data errors. 
     * @param amount Number of items to build. 
     */
    public void newShipyardOrder(int amount) {
        try {
            ...
            else if(main.getMainControler().maxShipItems(main.getSelectedPlanet().getPlanetPK(), Location.P, item) == 0)
                JOptionPane.showMessageDialog(main, "Too few resources!", "Cannot start", JOptionPane.ERROR_MESSAGE);
            else if(main.getMainControler().maxShipItems(main.getSelectedPlanet().getPlanetPK(), Location.P, item) < amount) {
                if(JOptionPane.showConfirmDialog(main, 
                        "Max possible number of " + item.toString() + " is " + 
                                main.getMainControler().maxShipItems(main.getSelectedPlanet().getPlanetPK(), Location.P, item) + 
                                ", build?") == JOptionPane.YES_OPTION) {
                    newShipyardOrderUnsafe();
                }
            }
            else
                newShipyardOrderUnsafe();
    ...
    /**
     * Starts building of new item. Does not check item technical requirements and input data errors. 
     */
    void newShipyardOrderUnsafe() {
        try {
            main.getMainControler().newShipOrder(main.getSelectedPlanet().getPlanetPK(), Location.P, item, parseIntSafe(amountTextField.getText()));
            main.getMaterials().updateMaterials();
    ...
    private void initComponents() {
        ...
        shipyardPanel.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(102, 102, 102)));
        shipyardPanel.setLayout(new java.awt.GridLayout(1, 3));

        iconLabel.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        iconLabel.setText("jLabel29");
        shipyardPanel.add(iconLabel);
        ...
        shipyardPanel.add(descriptionPanel);
        ...
        shipyardPanel.add(startPanel);

        add(shipyardPanel, java.awt.BorderLayout.CENTER);
    ...
    private void startButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_startButtonActionPerformed
        ...
        if(value.isEmpty())
            ...
            newShipyardOrder(Integer.valueOf(value));
